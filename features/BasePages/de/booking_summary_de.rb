...
class BookingSummaryPage < BookingSummaryBasePage
  ...
  def verify_booking_summary_details
    title, value=$g_booking.get_booking_summary("bookingCode")
    assert_wait_for_text value
    check_partial_text_shown title
    title, value=$g_booking.get_booking_summary("leadPassenger")
    scroll_page_and_assert_text value
    check_partial_text_shown title
    title, value=$g_booking.get_booking_summary("otherPassengers")
    value.each do |var|
