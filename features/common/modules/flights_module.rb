  ...
  module Phone
    ...
    module Deu
      ...
      def check_flight_biscuit
        ...
        assert_text_present flight_data["arrivalAirportCode"]
        assert_text_present flight_data["arrivalAirportName"]
        @travel_month = DateTime.parse(flight_data["departureDateTime"]).strftime "%B"
        @de_travel_month= CommonMethods.new.get_month_translated @travel_month
        @travel_date = DateTime.parse(flight_data["departureDateTime"]).strftime "%0d. #{@de_travel_month} %Y"
        assert_partial_text @travel_date
      end
    end
  end
